...
import { GAgentClient } from "@/lib/gagent-client";
...
const SIDEBAR_STORAGE_KEY = "g-agent-webui.sidebar";
...
function Shell() {
  ...
  const [controlRoomOpen, setControlRoomOpen] = useState(true);
  ...
  return (
    <div className="relative flex h-full w-full overflow-hidden text-foreground">
      ...
      <main className="flex h-full min-w-0 flex-1 flex-col relative">
        <ThreadShell
        ...
        <div className="absolute top-4 right-4 z-30">
          <Button variant="outline" size="sm" onClick={() => setControlRoomOpen(!controlRoomOpen)}>
             {controlRoomOpen ? "Hide Control Room" : "Show Control Room"}
          </Button>
        </div>
      </main>
      ...
      <aside
        ...
        style={{ width: controlRoomOpen ? 320 : 0 }}
      >
        <div className="absolute inset-y-0 right-0 h-full w-[320px] overflow-hidden">
          <ControlRoom />
        </div>
      </aside>
