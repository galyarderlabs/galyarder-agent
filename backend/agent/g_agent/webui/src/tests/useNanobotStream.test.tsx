...
import { useGAgentStream } from "@/hooks/useGAgentStream";
...
function wrap(client: ReturnType<typeof fakeClient>["client"]) {
  return function Wrapper({ children }: { children: ReactNode }) {
    return (
      <ClientProvider
        client={client as unknown as import("@/lib/gagent-client").GAgentClient}
        token="tok"
      >
        {children}
      </ClientProvider>
...
describe("useGAgentStream", () => {
  it("collapses consecutive tool_hint frames into one trace row", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useGAgentStream("chat-t", []), {
  ...
  it("attaches assistant media_urls to complete messages", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useGAgentStream("chat-m", []), {
      wrapper: wrap(fake.client),
    });
