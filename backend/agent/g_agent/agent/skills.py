...
class SkillsLoader:
    ...
    def _parse_skill_metadata(self, raw: str) -> dict:
        """Parse g-agent skill metadata JSON from frontmatter."""
        try:
            data = json.loads(raw)
            if not isinstance(data, dict):
                return {}
            if isinstance(data.get("g_agent"), dict):
                return data.get("g_agent", {})
            return {}
    ...
    def _get_skill_meta(self, name: str) -> dict:
        """Get g-agent metadata for a skill (cached in frontmatter)."""
