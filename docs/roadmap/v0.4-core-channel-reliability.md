...
- [ ] Shared channel capability flags are not implemented.
- [ ] Normalized media envelope types are not implemented.
...
- [ ] Channel capability flags are available to the agent loop.
...
1. Add channel capability flags.
   - Expose through `/status` and maybe `MessageTool`.
2. Add normalized media envelope.
   - Existing `InboundMessage.media` is `list[str]`; keep it but add metadata
     fields for richer refs.
...
- `test_channel_capabilities.py`
- `test_media_envelope.py`
