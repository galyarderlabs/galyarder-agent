...
The owner can define and inspect character profiles with stable identity, voice,
boundaries, relationship model, channel behavior, visual identity, and proactive
behavior policy. Full live switching isolation remains follow-up work.
...
- Character/profile coverage is currently spread across command, profile, and
  visual tests; dedicated `test_character_profile.py` and `test_visual_identity.py`
  are still useful follow-ups.
...
- `character/profile.py`, `character/store.py`, and `character/visual.py` now
  exist.
- `SelfieTool` still reads visual identity from global config, not a merged
  per-character profile config.
