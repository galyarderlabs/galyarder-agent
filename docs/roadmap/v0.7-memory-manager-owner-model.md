# v0.7: Memory Manager And Owner Model [NOT SHIPPED]
...
- [ ] No `g_agent/memory/manager.py` exists.
- [ ] No `MemoryManager` or `MemoryProvider` abstraction exists.
- [ ] External memory provider selection is not implemented.
- [ ] Memory lifecycle hooks are not formalized through a manager.
...
- [x] Memory recall is fetched before prompt assembly through `MemoryStore`.
- [x] Memory is placed under the runtime metadata section.
- [x] Owner can update facts through current memory tools.
- [ ] Memory blocks use the planned provider-level fencing.
