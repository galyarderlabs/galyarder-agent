# v0.8: Owner-Reviewed Learning Loop [FIRST SLICE]
...
- [ ] Accept/reject/edit/apply lifecycle is not fully implemented.
- [ ] Background reviewer hook is not implemented.
- [ ] Rollback metadata is not wired to apply flows.
...
- [ ] Owner can accept, reject, and edit candidates.
...
- `LearningQueue` and `LearningCandidate` exist.
- `/learn` exposes queue list/inspect through chat commands.
...
4. Add accept/reject/edit.
   - Accept memory candidate writes through the current `MemoryStore` until
     v0.7 introduces `MemoryManager`.
