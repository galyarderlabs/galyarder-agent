...
- Add a dedicated `security/approval_policy.py` classifier test matrix for risky
  shell and filesystem examples.
...
- User-facing approval flow has first-class `/approve` pass-through, `/deny`
  ...
  It still needs narrow allowlists and approval policy classifiers.
