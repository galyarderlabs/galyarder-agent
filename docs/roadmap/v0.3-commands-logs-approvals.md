...
- Persist approval decisions into a first-class approval state table/file rather
  than only session metadata.
...
- User-facing approval flow has first-class `/approve` pass-through and `/deny`
  session-metadata clearing. It still needs first-class persisted approval state
  and narrow allowlists.
